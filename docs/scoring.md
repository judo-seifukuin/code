...
- **目標スコア**: **85点以上**（コア4指標がすべて正常域 or 軽度に収まる目安）
- **平均スコア**: 院内データの蓄積に応じて GAS の `stats` アクションが「あなたの平均」「院内平均」を返却します
- **前回比**: 最新の保存済みスコアからの差分を結果画面に「+X点 / -X点」で表示

---

## 4.2 姿勢一言サマリ（summaryLabel）

結果画面に「つまり何という姿勢か」を1行で提示するために、最重症の指標から命名します:

- 全項目が正常域: **「ほぼ理想的な正面立位」**（95点以上）or **「概ね均整のとれた正面立位」**
- 1項目のみ異常: **「軽度な右上がりの肩傾向」** など
- 2項目異常: **「中等度な右上がりの肩・骨盤左へ傾斜傾向」** など
- 3項目以上: **「複合姿勢（右上がりの肩・体幹左へ傾斜 など3項目）」**

修飾語は最重症項目の重症度（軽度 / 中等度 / 顕著）から取得します。
